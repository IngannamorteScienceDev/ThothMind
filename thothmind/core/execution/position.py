...
def build_long_only_positions(
    ...
    *,
    ...
    if pred_col not in out.columns:
        raise KeyError(f"Prediction column '{pred_col}' not found.")

    ret_col = resolve_return_col(out)
    close_col = resolve_close_col(out)

    pred = _coerce_numeric(out[pred_col])
    lower = _coerce_numeric(out[lower_bound_col]) if lower_bound_col and lower_bound_col in out.columns else pred
    ...
    realized_vol = daily_ret.rolling(policy.vol_lookback, min_periods=max(5, policy.vol_lookback // 3)).std()
    ...
    edge = pred - enter_threshold
    raw_conf = ((edge / max(enter_threshold, 1e-9)) + 1.0).clip(
        lower=policy.confidence_floor,
        upper=policy.confidence_cap,
    )

    desired_weight = (raw_conf * vol_scale).clip(lower=policy.min_position, upper=policy.max_position)

    enter_ok = (lower > enter_threshold) & trend_ok & (~blocked)
    exit_ok = (pred < exit_threshold) | (~trend_ok) | blocked
    ...
    for i in range(len(out)):
        ...
        if not in_pos:
            ...
            # optional rebalance while in trend, but do not churn hard
            current_weight = max(float(policy.min_position), float(desired_weight.iloc[i]))
    ...
    out["position"] = pd.Series(decision_weight).shift(int(policy.execution_lag)).fillna(0.0).clip(
        lower=policy.min_position,
        upper=policy.max_position,
    )
    out["signal"] = ((pd.Series(decision_weight) > 0).astype(int) - (pd.Series(decision_weight).shift(1).fillna(0) > 0).astype(int)).clip(lower=0)

...
def _infer_feature_cols(df: pd.DataFrame, target_col: str) -> list[str]:
    ...
    if not feature_cols:
        raise ValueError("No numeric feature columns inferred for conformal walk-forward stage.")
...
def _split_train_and_calibration(train_df: pd.DataFrame, calib_frac: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    calib_frac = min(max(float(calib_frac), 0.05), 0.40)
    calib_size = max(20, int(round(len(train_df) * calib_frac)))
    if len(train_df) - calib_size < 50:
        calib_size = max(10, min(calib_size, len(train_df) // 3))
    proper_train = train_df.iloc[: len(train_df) - calib_size].copy()
    calib = train_df.iloc[len(train_df) - calib_size :].copy()
...
def run_walkforward_ml_conformal_oos(
    ...
    **_: Any,
    ...
    Main upgrade:
    - enter based on lower prediction bound, not raw point forecast
    """
    ...
    conformal_alpha = float(model_cfg.get("conformal_alpha", 0.10))
    ...
    for window_id, (tr_s, tr_e, te_s, te_e) in enumerate(slices, start=1):
        ...
        proper_train, calib_df = _split_train_and_calibration(train_df, calibration_frac)
        ...
        pred_parts.append(
            pred_span[
                [c for c in ["date", "window_id", "pred", "pred_lo", "pred_hi", "conf_radius", "y_true", target_col, ret_col, "market_regime"] if c in pred_span.columns]
            ].copy()
        )
    ...
    result = WalkforwardConformalResult(
